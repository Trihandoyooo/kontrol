...
<div class="card-container mt-4">
    <div class="page-heading mb-3">
        <h3>Detail Outcome</h3>
        <p class="text-muted mb-4">Menampilkan detail kegiatan outcome dan status verifikasi.</p>
    </div>

    {{-- Informasi --}}
    <div class="section-title">Informasi Outcome</div>
    <table class="data-table mb-4">
        <tr><td class="data-label">Judul</td><td class="data-value">{{ $outcome->judul }}</td></tr>
        ...
        <tr><td class="data-label">User</td><td class="data-value">{{ $outcome->user->name ?? '-' }}</td></tr>
    </table>
    ...
    <form method="POST" action="{{ route('admin.outcome.updateStatus', $outcome->id) }}">
        @csrf
        @method('PATCH')
        ...
        <div class="mb-3">
            ...
            <select name="status" id="status" class="form-select" required onchange="toggleAlasanTolak(this.value)">
                <option value="terkirim" {{ $outcome->status == 'terkirim' ? 'selected' : '' }}>Terkirim</option>
                <option value="diterima" {{ $outcome->status == 'diterima' ? 'selected' : '' }}>Diterima</option>
                <option value="ditolak" {{ $outcome->status == 'ditolak' ? 'selected' : '' }}>Ditolak</option>
            </select>
        </div>

        <div class="mb-3" id="alasan_tolak_container" style="display: {{ $outcome->status == 'ditolak' ? 'block' : 'none' }};">
            <label for="alasan_tolak" class="form-label">Alasan Tolak</label>
            <textarea name="alasan_tolak" id="alasan_tolak" class="form-control" rows="3">{{ old('alasan_tolak', $outcome->alasan_tolak) }}</textarea>
        </div>

        <button type="submit" class="btn btn-outline-primary">Simpan Status</button>
        <a href="{{ route('admin.outcome.index') }}" class="btn btn-outline-secondary">Kembali</a>
    </form>
</div>
